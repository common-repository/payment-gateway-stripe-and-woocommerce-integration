...
class EH_Stripe_Payment extends WC_Payment_Gateway {
            ...
            if(Eh_Stripe_Admin_Handler::wtst_oauth_compatible($mode)){
                if ('test' === $mode) { 
                    if('wtst_oauth_expriy' === get_transient( 'wtst_oauth_expriy_test')){ 
                        ...
                        return self::wtst_refresh_token(0, 1);
                    }
     ...
     * @param $retry_count int current retry count
     * @param $max_retries int how many times retry this function if timeout error occurs
     * @return refresh token and access token
     * @since 4.0.0
     * 
     */
    public static function wtst_refresh_token($retry_count = 0, $max_retries = 1)
    {
        try{
            ...
            if('test' === $stripe_settings["eh_stripe_mode"]){ 
                $stripe_secret_key = 'sk_test_51Oqk92LdIlnHGLasJelZtxDthEhdOXYQ1vMRGQNWhAKMalInYE3cQsi5y2INIgcy0n0FnEiO7EguGaI3drtmuwMp00o50U6JKE'; // Replace *** with your actual secret key
                $refresh_token = get_option("wt_stripe_refresh_token_test");
            }
            else{ 
                $stripe_secret_key = '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'; // Replace *** with your actual secret key
            ...
            // Endpoint URL
            $url = 'https://api.stripe.com/v1/oauth/token';
            ...
            $args = array(
                ...
                'method'      => 'POST',
                'timeout'     => apply_filters("wtst_refresh_token_timeout", 45),
            );
             wc_get_logger()->log('Refresh token API request', print_r($args, true), array( 'source' => 'wt_stripe_oauth' )  );

            // Perform the POST request
            $response = wp_remote_post($url, $args);
            
             wc_get_logger()->log('Refresh token API response', print_r($response, true), array( 'source' => 'wt_stripe_oauth' )  );
            ...
            if (is_wp_error($response)) {
                ...
                if (false !== strpos($error_message, 'cURL error 28') && $retry_count < $max_retries) {
                    wc_get_logger()->log('Refresh token API retry', "Attempt " . ($retry_count + 1), array('source' => 'wt_stripe_oauth'));
                    
                    // Wait for a short time before retrying (e.g., 2 seconds)
                    sleep(2);
                    
                    // Retry the function call
                    return self::wtst_refresh_token($retry_count + 1, $max_retries);
                }
                else{
